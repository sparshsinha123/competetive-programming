...
int main(){
    ...
    while(t--){
       ...
       for(int i = 0; i < n; i++){
        ...
        // define a function to check if it is possible to take
        // mid as the value
        auto possible_to_take = [&](ll mid) -> bool{
          ll mnsum = 0;
          ll cntGreaterOrEquals = 0; // there must be exactly (n+1)/2 values >= x
          ll cntStrictlyLess = 0;
          for(int i = 0; i < (int) lr.size() ; i++){
            if(lr[i].second < mid){
                mnsum += lr[i].first;
                cntStrictlyLess++;
            } else if(mid <= lr[i].first){
                mnsum += lr[i].first;
                ++cntGreaterOrEquals;
            } else{ // mid > lr[i].first && mid <= lr[i].second
                // the minimum value i can take is lr[i].first and maximum is
                // mid
                if(cntGreaterOrEquals < (n + 1) / 2) {
                  mnsum += mid;
                  cntGreaterOrEquals++;
                } else { 
                  mnsum += lr[i].first;
          ...
          if(cntGreaterOrEquals >= (n + 1) / 2 &&
             cntStrictlyLess <= (n / 2) &&  mnsum <= s && mnsum >= foo) return true;
          return false;
        };
       ...
       reverse(all(lr));
        ...
        ll low = dx , high = bar;
        ll ans = -1;
        // start binary search for answer
       while(low <= high){
          // focus on the midpoint
           ll mid = (low + high) >> 1; 
          
           if(possible_to_take(mid)){ // if it is possible to take
              ans = mid; // save the answer
              low = mid + 1; // look in the right half for greater values
           } else{ // reduce mid and check
               high = mid - 1;
           }
       }
