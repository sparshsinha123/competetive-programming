...
int main(){
    ...
    for(int test = 1; test <= NTC; ++test){
        ...
        debug(path);
        for(int down1 = 0; down1 <= H - 1; down1++){
            if(down1 != 0 && L == 1) continue;
            int x = 1 + down1;
            if(x < U || x > D){
                for(int down2 = 0; down2 <= H - 1 - down1; ++down2){
                    int xx = x + down2;
                    if( (x < U && xx < U) || (x > D && xx > D) ){
                        int down3 = (H - 1 - down1 - down2);
                        if(R == W && down3 != 0) continue;
                        long double f = 1;
                        int curx = 1, cury = 1;
                        ans += path;
                    }
