...
int main(){
    ...
    while(q--){
         ...
         if(c==1){
             ...
             ll ans=-1;
             if(!dr.count(t)){
               auto it=dr.upper_bound(t);
               if(it==dr.end()) cout<<findXOR(t,n)<<'\n';
               else cout<<findXOR(t,it->first-1)<<'\n';
               continue;
             }
             while(low<=high){
                ll mid=(high+low)/2;
                if(o_set.find(mid)==o_set.end()){
                  high=mid-1;
                } else if(o_set.order_of_key(mid+1)-o_set.order_of_key(t)==mid+1-t){
                  ans=mid;
                  low=mid+1;
                } else{
                  high=mid-1;
             ...
             ll k=ans+1;
             auto it=dr.upper_bound(k);
             if(it==dr.end())cout<<findXOR(k,n)<<'\n';
             else cout<<findXOR(k,it->first-1)<<'\n';
         }  
    } 
}
