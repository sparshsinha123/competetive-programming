...
int main(){
    ...
    while(t--){
        ...
        for(int i=1; i<m; i++){
            ll oper = k[i-1];
            ll cost = dp[i-1];
            ll alone = (oper+1)*(lengths[i].S) + (oper+1)*(dp[i-1]/k[i-1]);
            ll v=lengths[i].F-(lengths[i-1].F+lengths[i-1].S-1)-1;
            ll comb= oper*(lengths[i].S)+ oper*(v+dp[i-1]/k[i-1]);
            // cerr<<"oper="<<oper<<" v="<<v<<" alone="<<alone<<" and comb="<<comb<<'\n';
            if(alone==comb){
                // cerr<<"alone=comb found\n";
                dp[i]=alone;
                k[i]=oper;
            }else if(alone<comb){
                dp[i]=alone;
