...
int main(){
    ...
    for(int t = 1; t <= tc; t++){
        ...
        int n;
        cin >> s >> n;
        vector<int> pref((int) s.length() , 0);
        vector<int> upto((int) s.length() , 0);
        ...
        for(int i = 0; i < (int)s.length(); i++){
            ...
            if(i != 0) upto[i] = max(upto[i - 1] , pref[i]);
            else upto[i] = pref[i];
        }
        ll ans = 0;
        // debugv(pref);
        for(int i = 0; i < (int) s.length(); i++){
            int low = i , high = (int) s.length() - 1;
            int p = (int) s.length();
            int bf = (i != 0 ? pref[i - 1] : 0);
            while(low <= high){
                int mid = (high + low) >> 1;
                if(pref[mid] - bf >= n){
                    p = mid;
            ...
            cerr << "for i = " << i << " found " << p << '\n';
            
            ans += 1ll * ((int) s.length() - p);
            debug(ans);
        }
        cout << ans << '\n';
    }
