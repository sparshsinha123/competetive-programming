...
int par[maxn] , cnt[maxn] , minel[maxn], e[maxn];
...
void merge (int x , int y){
    ...
    int py = findset(y);
    if (px == py) return;
    if (minel[px] == minel[py]){
        cnt[py] = cnt[px] + cnt[py];
        par[px] = py;
    } else{
        par[px] = py;

        cnt[py] = (minel[px] < minel[py] ? cnt[px] : cnt[py]);
        minel[py] = min (minel[px] , minel[py]);
...
void solve(){
   ...
   ll cost = 0 , ways = 1;
   for (int i = 1; i <= n ; i++){
    dsu.insert (findset(i));
   }
   // cout << "disjoint sets\n";
   // for (auto x: dsu){
   //  cout << "(" << x << "," << minel[x] << ","<< cnt[x] << ") ";
   // }
   // cout << endl;
   for (auto x : dsu){
    cost += minel[x];
