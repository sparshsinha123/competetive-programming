...
int main(){
    ...
    for(int i = 1; i <= n; i++){
        pq.push(MP(MP(dis[i] , -below[i]) , i));
    }
    ll ans = 0;
    while(k){
        pair<pi,int> p = pq.top();
        int dgr = p.first.second;
        int ds = p.first.first;
        int ver = p.second;
        ans = ans + 1ll*ds - 1ll*below[ver];
        // cerr << "vertex = " << ver << " ans = " << ans << " belw = " << below[ver] << " and dis = " << dis[ver] << " and ds = " <<'\n';
        pq.pop();
        k--;
    }
    cout << ans << '\n';
