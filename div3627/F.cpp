...
int dp[N][3];
int a[N];
void DfsU(int v, int parent){
    int S = 0;
    for(auto x : g[v]){
      if(x != parent){
        DfsU(x , v);
        if(dp[x][0] > 0) S += dp[x][0];
      }
    }
    if(a[v] == 1) dp[v][0] = 1 + S;
    else dp[v][0] = S - 1;
    cerr << " for vertex v = " << v << " set value = " << dp[v][0] << " and S = " << S << " AND a[v] = " << a[v] << '\n';
}
void DfsD(int v, int parent){
    int S = 0;
    if(parent == -1){
      if(a[v] == 0) dp[v][1] = -1;
      else dp[v][1] = 1;
    ...
    for(auto x : g[v]){
      if(x != parent){
        dp[x][1] = dp[v][0];
        if(dp[x][0] > 0) dp[x][1] -= dp[x][0];
        dp[x][1] += (a[x] == 1 ? 1 : 0);
    cerr << " for vertex x = " << x << " set value = " << dp[x][1]  << " AND a[x] = " << a[x] << '\n';
