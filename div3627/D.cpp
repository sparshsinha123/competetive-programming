...
int main(){
    PRELUDE;
     ...
     sort(all(c));
     ll Ans = 0;
     // to find pairs such that c[i] + c[j] > 0
     // for each c[i] we need to to find the number of j's such that c[j]>-c[i]
     // i.e all such j's for which c[j] >= -c[i] + 1 and j < i
    //  debugv(c);
     for(int i = 0; i < n; i++){
        if(c[i] <= 0) continue;
        int d = -c[i] + 1;
        // largest j for which value is c[j] < d
        int low = 0, high = i;
        int j = -1;
        while(low <= high){
          int mid = (low + high) >> 1;
