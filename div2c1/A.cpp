...
int main(){
    PRELUDE;
    int n , k , d;  cin >> n >> k >> d;
    // dp[i][j] is the number of ways of writing i as the sum of numbers with one number at least j 
    // such that each number <= k
    dp[1][0]=1; dp[1][1]=1; dp[0][0]=1;
    for(int i = 2; i <= n; i++){
        for(int j = 0; j <= min(i,k); j++){
             for(int l = 1; l <= k; l++){
                 if(l < j) dp[i][j] += ((i-l>=0) ? dp[i-l][j]:0);
                 else dp[i][j] += ((i-l>=0)?dp[i-l][0]:0);
             }
            //  cout<<"("<<i<< ","<< j << ")=" << dp[i][j] << " ";
        }
        // cout<<endl;
