...
int main(){
    PRELUDE;
    int n,p; cin>>n>>p; string s; cin>>s; s="#"+s;
    ll c=0;
    int mn=-1,mx=-1;
    for(int i=1;i<=n/2;i++){
       if(s[i]!=s[n-i+1]){
    ...
    if(mn==-1) cout<<0<<endl;
        ...
        int l1=n-mx+1; int l2=n-mn+1;
        if(p<=mn) c += 1ll*(mx-p);
        else if(p>mn && p<=mx) c += min(1ll*(mx-p)+1ll*(mx-mn),1ll*abs(mn-p)+mx-mn);
        else if(p>mx && p<=n-mx+1) c+= 1ll*min(abs(p-mn),abs(l2-p));
        else if(p>n-mx+1 && p<=n-mn+1) c+= 1ll*min(abs(l2-p),abs(l1-p))+abs(mx-mn);
        else if(p>n-mn+1) c+= abs(p-(n-mx+1));
        cout<<c<<endl;
    }
