...
int main(){
    ...
    //     cout << '\n';
    // }
    if(move <= k && k <= head_swaps){
        vector<int> index((int) v.size());
        int rem = (k - move);
        for(int i = 0; i < (int) v.size(); i++){
            if((int)v[i].size() == 1) {
                index[i] = 0;
                ...
                if(rem >= (int)v[i].size() - 1){
                    index[i] = 0;
                    rem -= ( (int)v[i].size() - 1);
                } else{
                    //rem<(s-1)
                    index[i] = ((int)v[i].size() - rem);
                    rem = 0;
                }
            }
        }
        ...
        for(int i = 0; i < (int) v.size(); i++){
            ...
            if((int)vo.size()!=0)ans.pb(vo);
            for(int j=index[i];j<(int) v[i].size();j++){
                ans.pb({v[i][j]});
            }
        }
