...
ll rec(ll N, int ty){
    if(ty==1){
        //same same
        if(N==3) return 0;
        ll p1=rec(N/2,2);//same dif
        ll p2=rec(N/2,4);//diff same
        ll p3=rec(N/2,1); //same same
        ll p4=(p1*p2)%mod;
        p3=(minv(m)* ((p3*(m-1))%mod))%mod;
        ll ans = ((p1*p2)%mod+(p3*p1)%mod)%mod;
        return ans;
    } 
    if(ty==2){
        //same diff
        if(N==3) return (m *(m-1))%mod; 
        ll p1=rec(N/2,2);//same dif
        ll p2=rec(N/2,3);//diff diff
        ll p3=rec(N/2,1); //same same
