...
ll get(ll x){
   ...
   for(ll d = 1; d <= mx; ++d){
      ll nd , c , ex = 0;
      if(d != mx) nd = power(10ll , d) - power(10ll , d - 1);
      else nd = x - power(10ll , d - 1) + 1;
      c = (nd * (nd + 1) / 2) * d;
      ll t = power(10ll , d ) - 1;
      if(d != mx) ex = (x - t) * nd * d;
      add += (c + ex);
...
ll getsum(ll x){
  ...
  ll ret = 0;
  for(int d = 1; dig >= d; ++d){
    ll p = ((d == dig) ? x : power(10ll , d)) - power(10ll , d - 1);
    if(d == dig) ++p;
    ret += p * d;
  }
  return ret;
}
int digitatindex(ll x , ll ind){
   vector<int> d;
   while(x){
      d.pb(x % 10);
      x /= 10;
