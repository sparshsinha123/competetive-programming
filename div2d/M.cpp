...
int n , m;
bool check(int f){
          // cerr << "called to check f = " << f << '\n';
          if(m % f != 0 || n % f != 0) return false;
          bool is1 = true , is2 = true , issame = true;
          for(int i = 0; i < f; ++i){
            if(s1[i] != s2[i]){
                  issame = false;
                  break;
          ...
          // cerr << "returning " << (is1 && is2 && issame) << '\n'; 
        return (is1 && is2 && issame);
}
int main(){
    PRELUDE;
       cin >> s1 >> s2;
