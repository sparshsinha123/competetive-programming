...
const int MAXN = 1005;
int n , m;
int dpU1[MAXN][MAXN] , dpU2[MAXN][MAXN] , dpD1[MAXN][MAXN] , dpD2[MAXN][MAXN] , a[MAXN][MAXN];
int main(){
    IOS;
    cin >> n >> m;
    for(int i = 1; i <= n; i++){
    ...
    for(int i = 1; i <= n; i++){
      for(int j = 1; j <= m; j++){
        dpU1[i][j] = a[i][j] + max(dpU1[i - 1][j] , dpU1[i][j - 1]);
      }
    }
    for(int i = 1; i <= n; i++){
      for(int j = m; j >= 1; j--){
         dpU2[i][j] = a[i][j] + max(dpU2[i - 1][j] , dpU2[i][j + 1]);
      }
    }
    for(int i = n; i >= 1; i--){
      for(int j = 1; j <= m; j++){
        dpD2[i][j] = a[i][j] + max(dpD2[i + 1][j] , dpD2[i][j - 1]);
      }
    }
    for(int i = n; i >= 1; i--){
      for(int j = m; j >= 1; j--){
        dpD1[i][j] = a[i][j] + max(dpD1[i + 1][j] , dpD1[i][j + 1]);
      }
    }
    int ans = 0;
    for(int x = 2; x < n; ++x){
      for(int y = 2; y < m; y++){
         ans = max(ans , dpU1[x - 1][y] + dpD1[x + 1][y] + dpU2[x][y + 1] + dpD2[x][y - 1]);
         ans = max(ans , dpU1[x][y - 1] + dpD1[x][y + 1] + dpU2[x - 1][y] + dpD2[x + 1][y]);
      }
    }
    cout << ans << '\n';
}