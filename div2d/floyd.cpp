...
int main(){
    ...
    Dfs(0);
    // dp[i][j][k] is the shortest distance between vertices i and j such that
    // the intermediate vertices in the path i--j are from the set 
    // {0 , 1, 2, ... , k - 1}
    
    // initialize the dp[i][j][0] meaning that intermediate path contains vertices
    // from the null set .. i.e paths like u--u or u->v (where u->v is an edge)
    for(int i = 0; i < n; i++){
      for(int j = 0; j < n; j++){
         dp[i][j] = G[i][j];
      }
