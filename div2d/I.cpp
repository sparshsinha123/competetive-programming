...
string getbin(int x){
  ...
  string ss;
  for(int i = v.size() - 1; i >= 0; i--) ss += (v[i] + '0');
  return ss;
}
ll rec(ll pos , ll BIT, int re){
     
     if(pos == bs.size()) {
       if(BIT==0)return 1;
       return 0;
     }
     if(dp[pos][BIT][re] != -1) return dp[pos][BIT][re];
     if(BIT == 0) return 1; // all 0s from now on 
     int cmin = 0, cmax = 1;
     int b = bs[pos] - '0';
     if(re == 0) cmax = b; // re = 0 means equality maintained uptil now so restrict choice
     ll res = 0;
     for(int c = cmin; c <= cmax; ++c){
        int nre = re; 
        if(c < b) nre = 1; 
        // if current choice is less than top then go unrestricted in 
        // future choices 
        if(c == 1) res += rec(pos + 1, BIT - 1, nre); // go for one less bit 
        if(c == 0) res += rec(pos + 1, BIT , nre); // same number of bits
     }
     return (dp[pos][BIT][re] = res);
}
ll count(ll n , ll b){
...
int main(){
    ...
    ll n , t;
    cin >> n >> t;
    if(!check(t)) cout << 0 << endl;
    else{
      ll p = log2(t) + 1;
      // to find number of m's in 1 .. n for which bitcount(m+1) is p
      // to find number of m+1s in 2..n+1 for which bitcount(m+1) is p
      debug(p);
      cout << count(n + 1 , p) - count(1, p) << endl;
    }
}