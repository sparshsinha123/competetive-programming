...
ll modexp(ll x, ll ex , ll MODULUS) {
    ll ans = 1ll;
    while (ex > 0) {
        if (ex & 1ll) ans = (ans * x) % MODULUS;
        ex >>= 1ll;
        x = (x * x) % MODULUS;
    }
    return ans;
...
int main(){
    ...
    map<string, bool> already_seen;
    vi pref((int)s.length() + 1, 0);
    for(int i = 1; i <= (int) s.length(); i++) pref[i] = pref[i - 1] + (t[s[i - 1] - 'a'] == '0');
    ...
    for(int i = 0; i < (int) s.length(); i++){
      for(int j = i; j < (int) s.length() ; j++){
        ...
        if(bad <= k && !already_seen.count(ts)){
          ++cnt; already_seen[ts] = 1;
        }  
      }
    }
    cout << cnt << '\n';
