...
/* dp[i][lf][rf][ls][rs] is the maximum xor from position p onwards given that 
 we have maintained the configuration <lf , rf, ls , rs> upto and including position
 p - 1 if it exists. 
 Configuration :
 lf = 0 means equality with l has been maintained so far for a
 lf = 1 strict inequality maintained for a
  and so on..
 */

string L , R;
ll solve(ll bitpos , int lf , int rf , int ls , int rs){ // answer for bitpos position  
        if(bitpos == L.size()) return 0;
        if(dp[bitpos][lf][rf][ls][rs] != -1) return dp[bitpos][lf][rf][ls][rs];
        ll res = 0;
        ll amin = 0, amax = 1, bmax = 1, bmin = 0;
        int lp = L[bitpos] - '0';
        int rp = R[bitpos] - '0';
        if(!lf) amin = lp;
        if(!rf) amax = rp;
        ...
        for(int a = amin; a <= amax; a++){
          for(int b = bmin; b <= bmax; ++b){
             ...
             ll t = 0;
             if(v){
                 ll pw = L.size() - bitpos - 1; 
                 t = (1ll << pw);
             }
             ll nlf = lf, nrf = rf , nls = ls , nrs = rs;
             if(!lf && a > lp) nlf = 1;
             if(!rf && a < rp) nrf = 1;
             if(!ls && b > lp) nls = 1;
             if(!rs && b < rp) nrs = 1;
             res = max(res , t + solve(bitpos + 1, nlf , nrf, nls , nrs));
          }
        }
        return (dp[bitpos][lf][rf][ls][rs] = res);
}
string bin(ll x){
  string s;
  while(x){
