...
int c;
int stk[N];
void dfs(int a, vector<bool> &vis, vll &A,vi &CYCLE,vector<int> &instk){
     vis[a] = true;
     stk[++c]=a;
     instk[a]=c;
     ll nx = (a+A[a]+1)%n;
     if(!vis[nx]){
         dfs(nx,vis,A,CYCLE,instk);
     } else{
        if(instk[nx]!=-1){
          for(int x=instk[nx]; x<=c;++x){
            if(CYCLE[stk[x]])break;
            CYCLE[stk[x]]=true;
            // cout<<"set x="<<stk[x]<<" "<<CYCLE[stk[x]]<<"\n";
          }
        } 
     }
     instk[a]=-1;
