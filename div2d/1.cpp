...
int main(){
    ...
    vector<int> dp(n,1);
    dp[0] = 1;
    unordered_map<int,int> max_dp;
    // vector<int> max_dp(MAXN , 1);
    getSpf();
    for(int i = 0; i < n; i++){
       int temp = a[i];
       vi pr;
      ...
      //  if(temp!=1)pr.pb(temp);
       ...
       while(temp != 1){
         int minimal_div = spf[temp];
         pr.pb(minimal_div);
         while(temp % minimal_div == 0) temp /= minimal_div;
       }
       ...
       dp[i]=1;
       for(auto p: pr){
         if(max_dp.count(p)) dp[i]=max(dp[i],1+max_dp[p]);
       }
       for(auto p: pr){
         max_dp[p]=max(max_dp[p],dp[i]);
       }
    }
    int ans=1;
    for(int i = 0; i < n; i++){
