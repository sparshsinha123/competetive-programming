...
int main(){
    ...
    int n , m;
    cin >> n;
    vector<int> A(n); // storing time to solve ith problem
    int sum = 0;
    for(int i = 0; i < n; i++){
       cin >> A[i];
    ...
    cin >> m;
    vector<pair<int, int>> TIME(m); // intervals 
    for(int i = 0; i < m; i++){
      cin >> TIME[i].first >> TIME[i].second; // taking input of the intervals
    }
    if(m == 0 || sum > TIME[m - 1].second){ 
      // if m is 0 or time taken to solve is more than the last time the website is up then
      // no matter how hard you try you can't submit
      cout << -1 << '\n';
    } else{
      // other wise find the first value of k for which either sum lies between 
      // lk -- rk or lk >= sum where [lk , rk] is the kth interval 
      int ans = -1 , k;
      for(k = 0; k < m; ++k){
        if(sum >= TIME[k].first && sum <= TIME[k].second){
          ans = sum;
          break;
        } else if(TIME[k].first >= sum){
           ans = TIME[k].first;
           break;
        }
      }
