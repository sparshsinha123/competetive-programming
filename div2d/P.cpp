...
string getStr(ll x){
  ...
  while(s.length()!=34)s.push_back('0');
  reverse(all(s));
  return s;
}
const int N=4000005;
...
map<char,ll> v[N];
static int a[N]; // number of leaves
static int b[N]; // number of strings from node onwards
static int yy=1;
...
void answer(string s){
      ...
      ll xo=0;
      int t=-1;
      for(int k=(int)ans.length()-1; k>=0;--k){
         t++;
         if(ans[k]=='1') xo = (xo|(1ll<<t));
      }
      cout<<xo<<'\n';
}
