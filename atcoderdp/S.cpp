...
int main(){
    ...
    int D;
    cin >> D;
    // dp[i][j][0] is the number of numbers with remainder of sum as j given that we
    // have not already choosen a smaller number 
    // dp[i][j][1] is the number of numbers with remainders of sum as j given that we have
    // already choosen a lesser digit somewhere [0..i]
    dp[0]
    for(int i = 0; i < n; i++){
        for(int j = 0; j < D; j++){
