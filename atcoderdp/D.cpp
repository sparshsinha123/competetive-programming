...
int main(){
    ...
    // dp[i][j] is the maximum value taking upto i items with weight at most j
    for(int i = 1; i <= n; i++){
        vector<long long> new_dp(w + 1, 0);
        for(int j = 0; j <= w; j++){
            new_dp[j] = max(dp[j] ,((j >= weight[i]) ? (dp[j - weight[i]] + value[i]): 0));
            if(j >= 1) assert(dp[j] >= dp[j - 1]);
        }
        dp = new_dp;
    }
    // for(int i = 1; i <= n; i++){
