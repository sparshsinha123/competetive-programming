...
int main(){
    ...
    // vector<vector<long long>> optimal(n , vector<long long>(n , 0));
    for(int i = n - 1; i >= 0; i--){
        for(int j = i; j < n; j++){
            if(i == j){
                dp[i][j] = 0;
            ...
            for(int ii = i; ii < j; ++ii){
                long long foo = pref[ii] - (i == 0 ? 0 : pref[i - 1]);
                long long bar = pref[j] - pref[ii];
                long long cost = foo + bar; 
                // dp[i][j] = min(dp[i][j] , dp[i][ii] + dp[ii + 1][j] + cost);
                if(dp[i][ii] + dp[ii + 1][j] + cost < dp[i][j]){
                    dp[i][j] = dp[i][ii] + dp[ii + 1][j] + cost;
                    // optimal[i][j] = ii;
                }
            }
        }
