...
int main(){
    ...
    while(t--){
        int n , k;
        cin >> n >> k;
        fenwick<int> fx(k + k + 1) , fn(k + k + 1);
        vector<int> a(n + 1) , cnt(k + k + 2);
        for(int i = 1; i <= n; i++) cin >> a[i];
        int u = n >> 1;
        for(int i = 1; i <= u; i++){
            int j = n - i + 1;
            fx.modify(max(a[i] , a[j]) , 1);
            fn.modify(min(a[i] , a[j]) , 1);
            cnt[a[i] + a[j]]++;
        }
        int mnop = inf;
        for(int x = k + k ; x >= 2; x--){
            mnop = min(mnop , u - cnt[x] + fx.get(x - k - 1) + fn.get(k + k + 1) - fn.get(x - 1));
        }
        cout << mnop << '\n';
    }      
}
