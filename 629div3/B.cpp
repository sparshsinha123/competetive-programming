...
string solve(ll n , ll bs , ll k){
   string s="";
  //  cerr<<"called with n="<<n<<"and k="<<k<<" and bs="<<bs<<'\n';
   if(n==0) return s;
   if(bs==0) return sss('a' , n);
   if(bs==2){
     ll tt = (n-1)*(n-2)/ 2;
        string s;
     if(k > tt){
        s.pb('b');
        return s + solve(n-1,1,k-tt);
     } else{
       s.pb('a');
       return s+solve(n-1,2,k);
     }
   } 
   if(bs==1){
      ll tt=(n-1);
      ...
      if(k>n-1){
        s.pb('b');
        return s+solve(n-1,0,k-tt);
      } else{
        s.pb('a');
        return s+solve(n-1,1,k);
      }
   }
   return s; 
}
