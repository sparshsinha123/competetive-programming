...
int main(){
    ...
    while(1){
          ...
          move = false;
          for(int q = 1; q <= p; ++q){
              while(!Q[q].empty() && Q[q].front().se.fi < round){
                pair<pi , pi> t = Q[q].front();
                int curx = t.fi.fi;
                int cury = t.fi.se;
                Q[q].pop();
                for(int d = 0; d < 4; ++d){
                  int xn = curx + dx[d];
                  int yn = cury + dy[d];
                  if(xn < 0 || xn >= n || yn < 0 || yn >= m || g[xn][yn] == '#' || c[xn][yn] > 0) {
                    continue;
                  }
                  move = true;
                  c[xn][yn] = q;
                  Q[q].push(MP(MP(xn , yn) , MP(round , s[q] - 1)));
                }
              }              
              while(!Q[q].empty() && Q[q].front().se.se != 0){
                pair<pi , pi> t = Q[q].front();
                int curx = t.fi.fi;
                int cury = t.fi.se;
                int sp = t.se.se; 
                ...
                for(int d = 0; d < 4; ++d){
                  ...
                  move = true;
                  c[xn][yn] = q;
                  Q[q].push(MP(MP(xn , yn) , MP(round , sp - 1)));
                }
              }
          }
          if(!move) break;
