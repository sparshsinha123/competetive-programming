...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        int low = 0, high = -1;
        for(int j = 0; j < m; j++){
            int k = (j > high ? 1 : min(high - j + 1, dp[i][low + high - j][0]));
            while(j - k >= 0 && j + k < m && a[i][j - k] == a[i][j + k]) ++k;
            dp[i][j][0] = k;
            if(j + dp[i][j][0] - 1 > high){
                high = j + dp[i][j][0] - 1;
                low = j - dp[i][j][0] + 1;
            }
        }
    ...
    for(int j = 0; j < m; j++){
        int low = 0, high = -1;
        for(int i = 0; i < n; i++){
            int k = (i > high ? 1 : min(dp[low + high - i][j][1] , high - i + 1));
            while(i - k >= 0 && i + k < n && a[i + k][j] == a[i - k][j]) ++k;
            dp[i][j][1] = k;
            if(i + dp[i][j][1] - 1 > high){
                high = i + dp[i][j][1] - 1;
                low = i - dp[i][j][1] + 1;
            }
        }
