...
int main(){
    ...
    for(int tt = 1; tt <= test_case; ++tt){
      ...
      bool bad = false;
      for(auto x : v){
         if(x.second.first == 1){ // a new activity comes up
            if(!j) {
              assign[x.second.second] = 'J';
              j = true;
            } else if(!c){
              assign[x.second.second] = 'C';
              c = true;
            } else{
              bad = true;
              break;
            ...
            assert(assign[x.second.second] != 'A');
            if(assign[x.second.second] == 'J') j = false;
            else c = false;
         }
      }
      cout << "Case #" << tt << ": ";
