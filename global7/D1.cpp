...
const int inf = 1e9 + 7;
int longestPalQuadratic(string &s){
    int longestPal = 1;
    ...
    IOS;
       ...
       while(t--){
         ...
         if(f >= l) cout << s << '\n';
             ...
             string mid = s.substr(f , l - f + 1);
             string rev = mid;
             reverse(all(rev));
             int l1 = longestPalQuadratic(mid);
             int l2 = longestPalQuadratic(rev);
            //  debug(l1); debug(l2);
             if(l1 > l2){
                for(int i = 0; i < f; i++) cout << s[i];
                for(int j = 0; j < l1; j++) cout << s[j + f];
                for(int j = l + 1; j < s.length(); j++) cout << s[j];
                cout << '\n';
             } else{
                for(int i = 0; i < f; i++) cout << s[i];
                for(int j = 0; j < l2; j++) cout << s[l - l2 + j + 1];
                for(int j = l + 1; j < s.length(); j++) cout << s[j];
                cout << '\n';
             }
         }
