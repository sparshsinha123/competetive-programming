...
ll totient(ll n, ll c){
    ...
    // debugv(pr);
    ll ans=c;
    for(ll mask=1;mask<(1<<pr.size());++mask){
        ll ct=__builtin_popcount(mask);
        ll b=bitcount(mask);
        ll tt=1;
        for(int j=0;j<b;j++){
            if(mask&(1<<j))tt*=pr[j];
...
int main(){
    ...
    while(t--){
        ...
        ll pp=m/g;
        ll k2=(a+m-1)/g;
        //to find number of numbers coprime to pp in range k1 to k2
        // debug(k2);debug(k1); debug(pp);
        
        if(k1==0)cout<<totient(pp,k2)<<'\n';
        else cout <<totient(pp,k2)-totient(pp,k1-1)<<'\n';
    }
}