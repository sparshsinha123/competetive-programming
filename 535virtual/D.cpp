...
bool check(string ans , string s , int out){
    int c = 0;
         ...
         for(int c = 0; c < 3; ++c){
            for(int pr = 0; pr < 3; ++pr){
               if(pr != c){
                 int g = 0;
                 if(code(s[i]) != c) g = 1;
                 dp[i][c] = min(dp[i][c] , dp[i - 1][pr] + g);
               }
            }
         }
       }
       int out = min({dp[n - 1][0] , dp[n - 1][1] , dp[n - 1][2]}); 
       cout << out << '\n'; 
       string ans = "";
       int last = -1;
       for(int i = n - 1; i >= 0; i--){
