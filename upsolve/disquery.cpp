...
class lca{
    ...
    lca(int n){
        ...
        minedge = vector<vector<int>>(MAX_D + 1, vector<int>(n + 1 , -1));
        maxedge = vector<vector<int>>(MAX_D + 1, vector<int>(n + 1 , -1));
    }
    ...
    void do_dfs(int sr){
        ...
        for(auto ver : g[sr]){
            ...
            if(!visited[child]){
                ancestor[0][child] = sr;
                minedge[0][child] = weight;
                maxedge[0][child] = weight;
        ...
        for(int d = 1; d <= MAX_D; d++){
            for(int i = 1; i <= n; i++){
                if(ancestor[d - 1][i] != -1){
                    ...
                    minedge[d][i] = min(minedge[d - 1][i] , minedge[d - 1][ancestor[d - 1][i]]);
                    maxedge[d][i] = max(maxedge[d - 1][i] , maxedge[d - 1][ancestor[d - 1][i]]);
                }
            }
        }
    }
    ...
    int get_max(int l , int d){
        int i = l;
        int mx = -inf;
        for(int dd = 0; (dd <= MAX_D && i != -1) ;dd++){
            if((1 << dd) & d) {
                mx = max(mx , maxedge[dd][i]);
                i = ancestor[dd][i];
            }
        }
        return mx;
    }
    int get_min(int l , int d){
        int i = l;
        int mn = inf;
        for(int dd = 0; (dd <= MAX_D && i != -1) ;dd++){
            if((1 << dd) & d) {
                mn = min(mn , minedge[dd][i]);
                i = ancestor[dd][i];
            }
        }
        return mn;
...
int main(){
    ...
    while(K--){
        int d , e;
        cin >> d >> e;
        int lc = l.get_lca(d , e);
        cout << min(l.get_min(d, l.depth[d]-l.depth[lc]),l.get_min(e, l.depth[e]-l.depth[lc]))<<" ";
        cout << max(l.get_max(d, l.depth[d]-l.depth[lc]),l.get_max(e, l.depth[e]-l.depth[lc]))<<"\n";
    }
    // cout << "Read\n";
}
