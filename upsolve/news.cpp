...
int main(){
    ...
    string s , t;
    cin >> s >> t;
    RollingHash RH1(s , PRIME1 , MOD);
    RollingHash RH2(t , PRIME1 , MOD);
    vector<int> idx;
    ll hashs = RH1.getHash(0 , (int) s.length() - 1);
    ll hasht = RH2.getHash(0 , (int) t.length() - 1);
    for(int i = 0; i < (int) s.length(); i++){
        ll myhash1 = (i - 1 >= 0 ? RH1.getHash(0 , i - 1) : 0);
        ll myhash2 = RH1.getHash(i + 1 , (int) s.length() - 1);
        ll myhash = (myhash1 + (pw[i] * myhash2) % MOD) % MOD;
        if(myhash < 0){
            myhash += MOD;
        }
        if(myhash == hasht){
