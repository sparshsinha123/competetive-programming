...
int main(){
    ...
    while(t--){
        ...
        int n = (int) s.length();
        RollingHash r1(s , primeNum1 , MOD);
        RollingHash r2(r , primeNum1 , MOD);
        int l = 0 , h = n - 1;
        while(l < h && s[l] == s[h]) {
            ++l;--h;
        }
        ...
        int R = h;
        for(int i = l; i <= h; i++){
            if(r1.getHash(l , i) == r2.getHash(n - 1 - i , n - 1 - l)) L = i;
        }
        for(int i = h; i >= l; i--){
            if(r2.getHash(n - 1 - h , n - 1 - i) == r1.getHash(i , h)) R = i;
        }
        debug(L);
        debug(R);
        if(L - l > h - R){
        ...
        //     // cout << s.substr(l , L - l + 1);
        //     // if(h != n - 1) cout << s.substr(h + 1);
        //     cout << '\n';
        } else{
        //     // if(l != 0) cout << s.substr(0 , l);
        //     // cout << s.substr(R , h - R + 1);
        //     // if(h != n - 1) cout << s.substr(h + 1);
        //     cout << '\n';
        }
    }
