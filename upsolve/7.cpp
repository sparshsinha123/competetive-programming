...
int cat[N];
int ans;
void dfs(int a , int par , int max_ending = cat[1],  int max_till_now = 0){
    bool leaf = true;
    for(auto x : g[a]){
        if(x != par){
            leaf = false;
            dfs(x , a , (cat[x] == 0 ? 0 :  (max_ending + 1)) , max(max_till_now , max_ending));
        }
    }
    if(leaf) ans += (max(max_ending , max_till_now) <= m);
}
int main(){
    IOS;
    cin >> n >> m;
