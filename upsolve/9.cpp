...
int main(){
    ...
    for(int i = 0; i < n; i++){
        a[i] = (ll) (lower_bound(b.begin() , b.end() , a[i]) - b.begin());
        ++a[i];
    }
    ll ans = 0;
    fenwick left , right;
    int invl = 0, invr = 0, invc = 0;
    for(int i = 0; i <= n - 2; i++) {
        invl += left.greaterthan(a[i]);
        left.modify(a[i] , 1ll);
    }
    int r = n - 1 , l = n - 2;
    while(r >= 0){
        while(l >= r){
            invl -= left.greaterthan(a[l]);
            invc -= right.lessthan(a[l]);
            left.modify(a[l] , -1);
            l--;
        }
        right.modify(a[r] , 1ll);
        invr += right.greaterthan(a[r]);
        invc += left.greaterthan(a[r]);
        while(l >= 0 && (invl + invr + invc) > k){
            invl -= left.greaterthan(a[l]);
            invc -= right.lessthan(a[l]);
            left.modify(a[l] , -1);
            l--;
