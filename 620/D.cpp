...
vector<int> longest(string &s){
  ...
  for(int ii=0; ii < (int)indices.size(); ii++){
     for(int j=0; j<(int)indices[ii].size();j++){
       v[indices[ii][j]]=smallest;
       mark[smallest]=1;
       ++smallest;
     }
  }
...
vector<int> shortest(string &s){
  ...
  int smallest=1;
  for(int ii=(int)indices.size()-1;ii>=0;ii--){
     for(int j=0; j<(int)indices[ii].size();j++){
       v[indices[ii][j]]=smallest;
       mark[smallest]=1;
       ++smallest;
...
int main(){
    ...
    while(t--){
      ...
      string s;
      cin >> s;
      vector<int> l = shortest(s);
      vector<int> r = longest(s);
      // cout << "string : " << s << '\n';
      for(auto x : l) cout << x << ' ';
      cout << '\n';
      for(auto x : r) cout << x << ' ';
      ...
      for(int i = 0; i < (int)s.length(); i++){
        if(s[i]=='<'){
            assert(l[i]<l[i+1]);
            // if (l[i]>l[i+1]){
            //   cerr<<"i = " << i << " l["<<i<<"] = " <<l[i]<<" l["<<i+1<<"] = " << l[i+1]<<'\n';
            // } 
            assert(r[i]<r[i+1]);
            // if(r[i]>r[i+1]){
            //   cerr<<"i = " << i << " r["<<i<<"] = " <<r[i]<<" r["<<i+1<<"] = " <<r[i+1]<<'\n';
            // }
        } else{
            assert(l[i]>l[i+1]);
            assert(r[i]>r[i+1]);
