...
int main(){
    ...
    for(auto x : str){
      string ss = x.first;
      int low = 0 , high = (int) ss.length()-1;
      bool bad = false;
      while(low < high){
         if(ss[low] != ss[high]){
           bad = true;
    ...
    for(auto x : str){
       ...
       string r = s;
       reverse(all(r));
       if(good.count(r) || good.count(s)) continue;
       if(pal.count(r)) continue;
       if(str.count(r) && str.count(s)){
         good[s]=1;
       }
    }
    // debugm(str);
    ...
    // debugm(good);
    vector<string>v,rv;
    for(auto x : good) v.pb(x.first);
    rv=v;
    reverse(all(rv));
    for(auto &x: rv) reverse(all(x));
    if((int)pal.size()!=0) v.pb(pal.begin()->first);
    cout << ((int)v.size() + (int)rv.size()) * m << '\n';
    for(auto x : v) cout<<x;
    for(auto y : rv) cout<<y;
    cout<<'\n';
}
