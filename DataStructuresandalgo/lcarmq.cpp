...
const int inf = 1e9 + 7;
const int N = 1e5 + 7;
int height[N] , euler[N<<1] , first[N] , sg[N<<2];
vector<int> g[N];
bool vis[N];
int c;
void do_dfs(int x , int h){
  vis[x] = true;
  height[x] = h;
  euler[c] = x;
  first[x] = c;
...
void build(int ss , int ee , int x){
    if(ss == ee){
       sg[x] = euler[ss]; 
       return;
    }
    int mid = (ss + ee) >> 1;
    build(ss, mid, x + x + 1);
    build(mid + 1, ee, x + x + 2);
    if(height[sg[x + x + 1]] < height[sg[x + x + 2]]) sg[x] = sg[x + x + 1];
    else sg[x] = sg[x + x + 2];
}

int query(int ss , int ee , int x , int qs , int qe){
  if(qs > ee || qe < ss) return -1;
  if(ss >= qs && ee <= qe) return sg[x];
  int mid = (ss + ee) >> 1;
  int l = query(ss , mid , x + x + 1, qs , qe);
  int r = query(mid + 1, ee, x + x + 2 , qs , qe);
  if(l == -1) return r;
  if(r == -1) return l;
...
int LCA(int u , int v){
  ...
  //  cout << "going to query minimum in range " << left << " , " << right << '\n';
   return query(0 , c - 1, 0 , left , right);
}
void lca_preprocess(){
  do_dfs(1 , 0);
  build(0 , c - 1, 0);
...
int main(){
    ...
    lca_preprocess();
    cout << "Euler: ";
    for(int i = 0; i < c; i++) cout << euler[i] << ' ';
    cout << '\n';
    cout << "height: ";
    for(int i = 1; i <= n; i++) cout << height[i] << ' ';
    cout << '\n';
    cout << "first: ";
    for(int i = 1; i <= n; i++) cout << "("<<i << ", "<<first[i] << ") ";
    cout << '\n';
    while(1){
