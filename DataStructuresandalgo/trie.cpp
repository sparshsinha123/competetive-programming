...
struct trienode* createNode(){
      struct trienode* new_node = new struct trienode;
...
struct trienode* insert(struct trienode* root , string s){
      if(root == nullptr) root = createNode();
      struct trienode* pcrawl = root;
      for(int i = 0; i < s.length(); ++i){
        int in = s[i] - 'a';
        if(pcrawl->children[in] == nullptr) pcrawl->children[in] = createNode();
...
void Dfs(struct trienode* root , string &s){
...
int main(){
    IOS;
    vector<string> v = {"sparsh","sanchit","xyz","corona","covid","coviddf"};
