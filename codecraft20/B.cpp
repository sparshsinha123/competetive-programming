...
const int inf = 1e9 + 7;
int n;
string solve(int choice , string &c){
       int k = choice - 0 + 1;
      for(int i = 0; i + k - 1 < n; i++){
             string p = c.substr(i , k);
            //  debug(c);
            // debug(p);
             reverse(p.begin() , p.end());
             string c1 = ((i != 0) ? c.substr(0 , i) : "");
             string c2 = (i + k != n ? c.substr(i + k) : "");
            //  debug(c1); debug(c2);
             c = c1 + p + c2;
          }
          return c;
    ...
    PRELUDE;
       ...
       while(t--){
          ...
          for(int i = 0; i < n; i++){
            char mnpos = s[i];
            for(auto x : ch){
              if(mark.count(x)) continue;
               int len = x + 1;
               if(i + len - 1 < n) {
                debug(s[i + len - 1]);
                 mnpos = min(mnpos , s[i + len - 1]);
               }
            }
            // cerr <<endl;
            for(auto x : ch){
