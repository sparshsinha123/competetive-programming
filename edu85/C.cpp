...
int main(){
    ...
    while(t--){
        ...
        vector<pll> clone(n);
        clone = a;
        // suppose we kill the 0th monster first
        int i = 0;
        while(i < n){
            cost += clone[i].first;
            int j = i + 1;
            while(j < n && clone[j - 1].second >= clone[j].first){
                j++;
            }
            if(j == n) break;
            clone[j].first -= clone[j - 1].second;
            i = j;
        } 
        // debug(cost);
        ll ans = cost;
        for(int i = 1; i < n; i++){
            // cout <<"i = " << i << '\n';
            ll my_cont = (a[i].first - min(a[i].first , a[i - 1].second));
            // debug(my_cont);
            ll new_cost = (cost - my_cont);
            new_cost += a[i].first;
            // debug(new_cost);
            int prev = i - 2;
            if(prev == -1) prev = n - 1;
            ll cur_cont = (a[i - 1].first - min(a[i - 1].first , a[prev].second));
            // debug(cur_cont);
            new_cost -= a[i - 1].first;
            // debug(new_cost);
            new_cost += cur_cont;
            // debug(new_cost);
            cost = new_cost;
