...
ll solve(ll n){
      ...
      for(int i = 1; i < d.size(); i++){
          if(d[i] == 0) ans += ans; // take all less and put 0 or 1
          else if(d[i] == 1) {
            ans += ans; // take all less and place 0 or 1 ..
            if(!seen) ans += 1; // with equal one place 0 .. can't place 1
          }
          else{
             ans += ans;
             if(!seen) ans += 2; // with equal one we can place anything
             seen = true;
          }
      }
      --ans; // uncount the 0 one
      if(seen) return ans; 
      else return ans + 1 ;
}
int main(){
