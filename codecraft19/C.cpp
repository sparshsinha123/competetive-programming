...
ll solve(ll st , ll en){
    if(st > en) return 0;
    ll nen = (ll)(upper_bound(a.begin() , a.end() , en) - a.begin());
    ll nst = (ll)(upper_bound(a.begin() , a.end() , st - 1) - a.begin());
    ll ele = nen - nst;
    if(ele == 0) return A;
    ll b = 1ll * B * ele * (en - st + 1);
    if(st == en) return b;
    ll mid = (st + en) >> 1;
    ll val = min(b , solve(st , mid) + solve(mid + 1 , en));
    return val; 
}
int main(){
    IOS;
