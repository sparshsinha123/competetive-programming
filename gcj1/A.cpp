...
int main(){
    ...
    for(int t = 1; t <= tc; t++){
        ...
        for(int i = 0; i < n; i++){
            ...
            if(j != 0)pref.push_back(v[i].substr(0 , j));
            if(j != (int) v[i].length() - 1) suff.push_back(v[i].substr(j + 1));
        }
        string mx_pref = "" ;
        int len = 0;
        for(auto x : pref){
        ...
        for(int i = 0; i < (int) pref.size(); i++){
            int k = 0;
            for(int j = 0; j < (int) pref[i].length(); j++){
                if(pref[i][j] != mx_pref[k]){
                    bad = true;
                    break;
                }
                k++;
        ...
        for(int i = 0; i < (int) suff.size(); i++){
            int k = (int) mx_suf.size() - 1;
            for(int j = (int) suff[i].length() - 1; j >= 0 ; j--){
                if(suff[i][j] != mx_suf[k]){
                    bad = true;
                    break;
                }
                k--;
